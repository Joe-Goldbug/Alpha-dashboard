...
def start_session() -> SingleSession:
    ...
    if os.environ.get("BRAIN_USE_WQOS_SESSION") == "1":
        wqos_root = os.environ.get("WQOS_ROOT", "/workspace/WQOS")
        if wqos_root not in sys.path:
            sys.path.insert(0, wqos_root)
        from src.sessions.session_client import get_session as _get_wqos_session

        wqos_session = _get_wqos_session()
        s = SingleSession()
        s.cookies.update(wqos_session.cookies)
        s.headers.update(wqos_session.headers)
        return s
